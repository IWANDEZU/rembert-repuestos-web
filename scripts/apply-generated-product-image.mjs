...
const OUTPUT_DIR = path.join(ROOT, "public", "catalogo-generated");
...
if (!inputFile) throw new Error("Uso: npm run catalog:generated-image -- <lote.json> [--apply]");
...
function labelSvg(product) {
  ...
  return Buffer.from(`
    <svg width="${OUTPUT_SIZE}" height="${OUTPUT_SIZE}" viewBox="0 0 ${OUTPUT_SIZE} ${OUTPUT_SIZE}" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="52" y="48" fill="#FFFFFF" font-family="Arial, sans-serif" font-size="30" font-weight="700">REMBERT · ${sku}</text>
      <text x="${OUTPUT_SIZE - 52}" y="48" fill="#CBD5E1" font-family="Arial, sans-serif" font-size="20" font-weight="700" text-anchor="end">REFERENCIA DE CATÁLOGO</text>
      ...
      <text x="${OUTPUT_SIZE / 2}" y="1164" fill="#F6C800" font-family="Arial, sans-serif" font-size="22" font-weight="700" text-anchor="middle">${title} · confirmar referencia y medidas antes de vender</text>
    </svg>
  `);
}
...
for (const [index, row] of batch.entries()) {
  ...
  if (REAL_IMAGE_STATUSES.has(product.imageStatus)) {
  ...
  prepared.push({ product, source, fileName, prompt });
}

if (!apply) {
  console.log(JSON.stringify({ valid: prepared.length, changesApplied: false, skus: prepared.map((item) => item.product.sku) }, null, 2));
  process.exit(0);
}
...
for (const item of prepared) {
  rendered.push({
    ...item,
    buffer: await renderImage(item.product, item.source),
    image: `/catalogo-generated/${item.fileName}`,
...
for (const item of rendered) {
  ...
  const entry = {
    image: item.image,
    images: [{ url: item.image, alt: `Imagen generada de referencia para ${item.product.name}, SKU ${item.product.sku}. No es fotografía original.`, isMain: true }],
...
const overrideSource = "// Generado por los importadores de imágenes del catálogo. No editar manualmente.\n"
...
console.log(JSON.stringify({ updated: rendered.length, imageStatus: "generated-reference-image", skus: rendered.map((item) => item.product.sku) }, null, 2));
